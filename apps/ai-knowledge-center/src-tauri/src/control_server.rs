...
pub(crate) const KNOWLEDGE_CENTER_APP_ID: &str = "ai-knowledge-center";
...
pub(crate) fn available_commands() -> Vec<AppCommandDescriptor> {
    vec![
        ...
        AppCommandDescriptor {
            id: "show-knowledge-health",
            title: "查看知识中心连接状态",
        },
        AppCommandDescriptor {
            id: "edit-knowledge-settings",
            title: "配置知识中心服务器",
        },
    ]
}
...
pub(crate) fn random_token(prefix: &str) -> String {
    ...
    getrandom::fill(&mut bytes).expect("failed to generate AI knowledge center token");
        ...
        .spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!(
                            "[ai-knowledge-center] {} connection failed: {error}",
                            server_name
                        );
                        break;
                    }
