    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            start_control_server(
                ...
                window_state_setup.clone(),
                ControlServerConfig {
                    name: "fw-ai-knowledge-center-control",
                    app_id: KNOWLEDGE_CENTER_APP_ID,
                    server_id: "fw-control",
                    bind_addr: "127.0.0.1:0",
                    token: random_token("kc-control"),
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) = start_backend(handle, state).await {
                    ...
                    eprintln!("[ai-knowledge-center] {error}");
                }
            });
            Ok(())
        })
        .build(context)
        .expect("error while building AI knowledge center app")
        .run(move |app, event| {
