...
impl BackendState {
    ...
    pub(crate) async fn endpoint(&self) -> Result<BackendEndpoint, String> {
        self.process.endpoint("AI 知识中心本地后台未就绪").await
    }
}
...
pub(crate) async fn start_backend(
    ...
    let session_token = random_token("kc-backend");
...
fn spawn_backend_child(
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| format!("启动 AI 知识中心本地后台失败: {e}"))?;
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| "AI 知识中心本地后台 stdout 不可用".to_string())?;
    Ok((child, stdout))
}
...
fn resolve_backend_sidecar(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    let exe_name = if cfg!(target_os = "windows") {
        "ai-knowledge-center-backend.exe"
    } else {
        "ai-knowledge-center-backend"
    };
    let candidates = [
        std::env::current_exe()
            .ok()
    ...
    Err(format!("AI 知识中心本地后台 sidecar 不存在: {exe_name}"))
}
