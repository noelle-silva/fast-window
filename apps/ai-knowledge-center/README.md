...
| --- | --- |
| 健康检查 | 已接入 |
| 顶部页面入口 | 已接入，包含收藏夹、全部笔记 |
| 维护页面入口 | 已接入，归档和回收站从设置页进入 |
| 全部笔记范围 | 已接入，只展示正常笔记，不混入归档和回收站 |
| 收藏夹范围 | 已接入，收藏夹按收藏内容展示，不沿用全部笔记的正常笔记范围 |
...
| 收藏夹路径条 | 已接入，收藏夹页面顶部显示当前位置，并支持点击跳到任意上级 |
| 页面位置记忆 | 已接入，每个页面和收藏夹内部位置独立保留 |
| 笔记正文渲染 | 已接入，笔记弹窗内正文按 Markdown 阅读形态展示，支持标题、列表、表格、代码块、公式和 Mermaid 图表 |

## 界面约定

当前界面使用纯色、边框和卡片组织信息，不使用渐变色风格。页面内部不再放置大型页头，连接状态和刷新入口收敛到顶部栏，主要空间留给笔记卡片和收藏夹卡片。点击笔记卡片后，完整内容在模态窗中展示，页面内部不再嵌入笔记详情区。

顶部栏只放常用浏览入口：收藏夹和全部笔记。全部笔记只展示正常笔记；收藏夹按收藏内容展示，不被全部笔记的范围规则误过滤；归档和回收站属于维护入口，从连接设置页面进入。

收藏夹页面顶部提供路径条。进入多级收藏夹后，路径条会展示从收藏夹首页到当前收藏夹的层级位置；点击路径中的上级收藏夹可以直接跳转，不需要逐级返回。

笔记弹窗内的正文使用从 AI Studio 消息阅读体验移植来的展示方式。正文不再以原始纯文本块展示，内容里的标题、换行、列表、表格、代码块、公式和 Mermaid 图表会按阅读形态呈现；代码块和图表源码支持复制，方便从笔记里继续整理内容。
