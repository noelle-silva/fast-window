...
class ClipboardHistoryDirectBackgroundClient implements DirectBackgroundClient {
  private activeEndpoint: any | null = null
  ...
  private readonly listeners = new Set<(event: DirectBackgroundEvent) => void>()
  ...
  invoke = async <T = unknown,>(method: string, params?: unknown): Promise<T> => {
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      this.markDisconnected(ws, new Error('剪贴板历史后台连接已关闭'))
      await this.ensureConnected()
    }

    const active = this.ws
    if (!active || active.readyState !== WebSocket.OPEN) throw new Error('剪贴板历史后台未连接')
    const id = `ch-${Date.now()}-${++this.seq}`
    return new Promise<T>((resolve, reject) => {
      ...
      try {
        ...
        this.pending.delete(id)
        clearTimeout(timer)
        this.markDisconnected(active, new Error('剪贴板历史后台连接已关闭'))
        reject(new Error('剪贴板历史后台连接已关闭'))
      }
    })
  }

  onEvent = (listener: (event: DirectBackgroundEvent) => void) => {
  ...
  close = () => {
    ...
    this.rejectPending(new Error('剪贴板历史后台连接已关闭'))
  ...
  private async ensureConnected(): Promise<void> {
    if (this.closed) throw new Error('剪贴板历史后台连接已关闭')
  ...
  private async reconnect(force: boolean) {
    ...
    if (force && this.ws) {
      const ws = this.ws
      this.ws = null
      this.rejectPending(new Error('剪贴板历史后台连接刷新中'))
      if (ws.readyState !== WebSocket.CLOSED) ws.close()
    }
    await this.ensureConnected()
  }
