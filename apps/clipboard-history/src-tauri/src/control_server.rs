...
pub(crate) fn session_token() -> String {
    ...
    getrandom::fill(&mut bytes).expect("failed to generate clipboard history control token");
        ...
        .spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!("[clipboard-history-app] {} connection failed: {error}", server_name);
                        break;
                    }
                }
            }
...
mod tests {
    ...
    fn accepts_matching_control_response_identity() {
        let body = r#"{"ok":true,"appId":"clipboard-history","serverId":"single-instance","protocolVersion":1}"#;
    ...
    fn rejects_mismatched_control_response_identity() {
        let body = r#"{"ok":true,"appId":"other","serverId":"single-instance","protocolVersion":1}"#;
        assert!(!control_response_matches(
            body,
            "clipboard-history",
            "single-instance"
