...
const INSTANCE_STATE_FILE: &str = "clipboard-history-single-instance.json";
...
fn instance_state_dir() -> PathBuf {
    if cfg!(target_os = "windows") {
        if let Some(dir) = env::var_os("LOCALAPPDATA").or_else(|| env::var_os("APPDATA")) {
            return PathBuf::from(dir).join("FastWindow").join("Clipboard History");
        }
    }
    ...
    env::temp_dir().join("fast-window-clipboard-history")
}
