...
const CONFIG_FILE: &str = "bookmarks-settings.json";
const WRITE_TEST_FILE: &str = ".fw-bookmarks-write-test";
...
pub(crate) fn save_data_dir(app: &tauri::AppHandle, data_dir: &Path) -> Result<(), String> {
    ...
    let settings = BookmarksSettings {
...
fn load_settings(app: &tauri::AppHandle) -> Result<BookmarksSettings, String> {
    let path = settings_path(app)?;
    if !path.is_file() {
        return Ok(BookmarksSettings::default());
    }
