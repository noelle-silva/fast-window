...
class BookmarksDirectBackgroundClient implements DirectBackgroundClient {
  ...
  invoke = async <T = unknown,>(method: string, params?: unknown): Promise<T> => {
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      this.markDisconnected(ws, new Error('网站收藏后台连接已关闭'))
      await this.ensureConnected()
    }

    const active = this.ws
    if (!active || active.readyState !== WebSocket.OPEN) throw new Error('网站收藏后台未连接')
    const id = `bm-${Date.now()}-${++this.seq}`
    return new Promise<T>((resolve, reject) => {
      ...
      try {
        ...
        this.pending.delete(id)
        clearTimeout(timer)
        this.markDisconnected(active, new Error('网站收藏后台连接已关闭'))
        reject(new Error('网站收藏后台连接已关闭'))
      }
    })
  }
  ...
  close = () => {
    ...
    this.rejectPending(new Error('网站收藏后台连接已关闭'))
    const ws = this.ws
    this.ws = null
  ...
  private async ensureConnected(): Promise<void> {
    if (this.closed) throw new Error('网站收藏后台连接已关闭')
  ...
  private async connect(): Promise<void> {
    ...
    ws.addEventListener('close', () => this.markDisconnected(ws, new Error('网站收藏后台连接已关闭')))
    ws.addEventListener('error', () => this.markDisconnected(ws, new Error('网站收藏后台连接异常')))
    try {
  ...
  private async reconnect(force: boolean) {
    ...
    if (force && this.ws) {
      const ws = this.ws
      this.ws = null
      this.rejectPending(new Error('网站收藏后台连接刷新中'))
      if (ws.readyState !== WebSocket.CLOSED) ws.close()
    }
    await this.ensureConnected()
  }
