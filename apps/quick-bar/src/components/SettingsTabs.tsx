...
export function SettingsTabs<T extends string>(props: SettingsTabsProps<T>) {
  ...
  const focusTab = React.useCallback((index: number) => {
    ...
    requestAnimationFrame(() => {
      document.getElementById(`quickbar-settings-tab-${item.id}`)?.focus()
    })
  }, [items, onChange])
  ...
  return (
    <div className="quickbar-settings-toolbar">
      <div className="quickbar-settings-tabs" role="tablist" aria-label={ariaLabel} onKeyDown={onKeyDown}>
        {items.map(item => {
          ...
          return (
            <button
              key={item.id}
              type="button"
              className="quickbar-settings-tab"
              role="tab"
              id={`quickbar-settings-tab-${item.id}`}
              aria-selected={selected}
              aria-controls={`quickbar-settings-panel-${item.id}`}
