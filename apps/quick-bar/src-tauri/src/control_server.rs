...
pub(crate) const QUICK_BAR_APP_ID: &str = "quick-bar";
...
pub(crate) fn random_token(prefix: &str) -> String {
    ...
    getrandom::fill(&mut bytes).expect("failed to generate quick-bar token");
        ...
        .spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!("[quick-bar] {} connection failed: {error}", server_name);
                        break;
                    }
                }
            }
