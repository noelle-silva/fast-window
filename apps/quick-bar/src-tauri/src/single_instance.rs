...
pub(crate) fn start_single_instance_server(
    ...
    let token = random_token("quick-bar-single");
    let endpoint = start_control_server(
        app,
        window_state,
        ControlServerConfig {
            name: "fw-quick-bar-single-instance",
...
fn write_instance_state(
    ...
    let state = InstanceState {
        app_id: QUICK_BAR_APP_ID.to_string(),
...
mod tests {
    ...
    use crate::control_server::QUICK_BAR_APP_ID;

    fn valid_state() -> InstanceState {
        InstanceState {
            app_id: QUICK_BAR_APP_ID.to_string(),
            desktop_identifier: "com.fastwindow.quickbar.dev".to_string(),
            server_id: SINGLE_INSTANCE_SERVER_ID.to_string(),
