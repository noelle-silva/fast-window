...
- 按钮图标：完整接入 Lucide 图标库，提供当前版本全部纯图标选项，注册和管理时都可以搜索、选择或随机图标
- 操作按钮：注册、编辑、保存、停用和删除使用统一现代按钮样式
- 结果展示：独立结果窗，不拉高按钮条；结果窗支持拖拽、复制结果、调整尺寸和记忆显示偏好
- 窗口外观：按钮条为圆角更收敛的白色矩形并带柔和外部阴影，结果窗为白色圆角窗口
- 窗口底色：外层保持透明，只显示白色圆角窗口本体
- 窗口轮廓：浮窗外圈不使用描边线
- 应用图标：使用低饱和米绿配色，表达划词、轻量按钮条和快速动作

## 工作方式

```mermaid
flowchart LR
  A[用户在外部软件选中文字] --> C[后端接住选区事实]
  C --> D{后端判断显示时机}
  D -->|选区后自动显示| F[桌面壳在选区附近显示按钮条]
  D -->|按快捷键显示| E[等待用户按快捷键]
  B[按下 Quick Bar 快捷键] --> G{后端现场确认当前选区}
  G -->|有当前选区| F
  G -->|没有当前选区| J[桌面壳保持隐藏]
  F --> L[外部点击、滚动或普通按键时退场]
  F --> K[用户点击已注册按钮]
...
Quick Bar 继续保持桌面壳、后端、管理页三层结构，但运行时职责已经归位：

- 后端负责接住选区事实、判断按钮条是否出现、处理按钮点击后的业务推进，并整理结果内容。
- 桌面壳负责实际显示和隐藏按钮条、结果窗，以及把外部动作和用户点击转交给后端。
- 管理页负责配置快捷键、显示时机、结果窗偏好和按钮管理，不承担按钮条运行主脑职责。

管理页关闭后，只要 Quick Bar 仍在后台运行，划词按钮条仍然可以按配置继续工作。
...
- Quick Bar 完整接入 Lucide 图标库，提供当前版本全部纯图标选项；注册按钮时可以搜索后选择，也可以用随机图标快速切换。
- 已注册按钮会保存图标信息；按钮管理页可以继续修改图标，不会影响名称、配置和启用状态。
- 图标选择面板支持按图标名称搜索；面板只显示图标，不在每个图标下面显示名称。
- 图标选择面板使用统一小尺寸图标墙，完整图标库通过滚动和搜索承载，避免图标撑开注册页或编辑弹窗。
- 按钮条只显示图标，鼠标悬停时才显示按钮名称，避免按钮条被文字撑宽。
- 鼠标经过按钮时，当前按钮的图标线段会以低饱和绿色高亮，便于确认当前指向。
- 按钮图标统一使用 1.25 的线条粗细，保证按钮条、图标墙和管理页视觉一致。
- 按钮条可见主体保底 160px 宽；按钮从左侧起排，按钮横向留出呼吸空间，超过保底后按真实图标内容继续加宽，避免图标按钮被整条栏平均拉宽。
...
能力调用结果和配置选项由 Fast Window 按统一标准交给 Quick Bar。Quick Bar 只负责展示和选择，不再自己拆解其他应用的原始回答。
...
Quick Bar 的浮动条唤醒快捷键由应用自己管理，不需要在 Fast Window 平台里为浮动条额外绑定快捷键。快捷键不是选区读取机制本身，而是默认的显示门槛：用户按下快捷键时，后端会现场确认当前是否仍有有效选区；有选区且能对应到实际划选位置时才让桌面壳显示浮动条，没有选区就保持隐藏。
...
Quick Bar 现在用统一取词路线接住“文字被选中了”这件事，再把这件事交给后端统一处理。后端会先等待外部软件的选区状态稳定，再读取当前可用的文字和位置，并决定是否需要让桌面壳显示按钮条。

- 统一取词路线拿到选中文字后，会把这次文字和位置直接交给当前显示流程。
- 按快捷键显示时，后端会在按键当下重新确认当前文字，不拿上一次文字冒充当前文字；显示位置仍使用实际划选时留下的位置。
- 取词路线会把文字、位置和来源一起交给 Quick Bar。
- 取词路线暂时拿不到结果时，会留下节制的失败原因记录，便于继续排查具体软件差异。
- 显示时机设为“选区后自动显示”时，后端会判断这次选区是否应该显示，并让桌面壳在选区附近显示按钮条。
- 显示时机设为“按快捷键显示”时，后端不会因为历史选区主动打扰用户。
- 取词只是入口，真正是否显示仍由显示时机决定。

统一取词路线直接由取词小帮手读取选中文字和位置，Quick Bar 后端使用同一套当前选区确认和显示时机，不维护第二套取词路线。

## 显示时机

Quick Bar 的后端选区观察和浮动条显示时机是分开的。

- 按快捷键显示：按下唤醒快捷键时现场确认当前文字，并配对实际划选位置；两者配上才显示浮动条。
- 选区后自动显示：后端发现新的有效选区后，决定显示浮动条，并让桌面壳在选区附近执行显示。
- 两种方式都使用同一套当前选区读取能力，不维护两套互相竞争的取词逻辑。
- 浮动条窗口本体就是按钮条，不再保留外壳套内条的视觉层级。
- 浮动条显示时不主动抢走原软件焦点，用户可以继续在原软件中复制或输入。
- 浮动条显示后不循环主动探测当前选区；用户点击外部、滚动或按普通按键时，画面立即退场。
- 浮动条内点击会保留当前按钮条操作，不会被外部动作退场规则误关。
...
- 默认数据目录由应用运行位置自动计算。
- Rust 后端负责 Quick Bar 自己的数据读写、按钮登记、健康检查、按钮点击业务处理和结果内容整理。
...
- 用户可以在注册时搜索并选择完整图标库里的图标，也可以用随机图标快速切换。
...
- 外部选中文字后，统一取词路线能够被后端接住，并在选区稳定后读取当前文字和位置。
...
- 鼠标悬停按钮时能看到按钮名称。
- 鼠标悬停按钮时，当前按钮的图标线段会以低饱和绿色高亮。
- 点击按钮后，独立结果窗准备好并出现在选区附近，按钮条随后隐藏。
- 能力调用中、调用成功、调用失败都有明确显示。
- 结果窗顶部栏可以拖拽窗口，并提供复制结果按钮和关闭按钮。
- 结果窗可以手动调整尺寸，调整后的尺寸会被记住。
- 结果窗可在设置中选择跟随选区附近显示，或固定在用户调整后的全局位置显示。
- 结果窗可在设置中选择手动关闭，或点击别处后直接消失。
- 结果窗顶部栏拖动属于窗口内部操作，不会触发“点击别处消失”。
- 结果窗关闭时画面立即退场，不展示系统窗口隐藏动画。
- 按钮条在点击其他地方、滚动、按普通按键或按 `Esc` 后立即退场。
- 按钮条退场不依赖循环读取当前选区，避免影响外部软件继续复制、粘贴或输入。
- 结果窗按 `Esc` 或关闭按钮后隐藏。
