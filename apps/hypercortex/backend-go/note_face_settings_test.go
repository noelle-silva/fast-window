...
func TestSaveNoteFaceSettingsPatchSemantics(t *testing.T) {
	...
	if _, err := svc.saveNoteFaceSettings("library", packageDir, "html", mustJSONRaw(t, map[string]any{"fixedScale": 0.8, "displayMode": "fit-window"})); err != nil {
	...
	// 只改缩放：显示方式必须保留。
	patched, err := svc.saveNoteFaceSettings("library", packageDir, "html", mustJSONRaw(t, map[string]any{"fixedScale": 0.5}))
	if err != nil {
		t.Fatalf("save face settings failed: %v", err)
	}
	...
	// null 删除缩放：显示方式仍然保留。
	cleared, err := svc.saveNoteFaceSettings("library", packageDir, "html", mustJSONRaw(t, map[string]any{"fixedScale": nil}))
	if err != nil {
	...
	// 非法显示方式被协议丢弃，合法值可改写。
	invalid, err := svc.saveNoteFaceSettings("library", packageDir, "html", mustJSONRaw(t, map[string]any{"displayMode": "weird"}))
	if err != nil {
	...
	valid, err := svc.saveNoteFaceSettings("library", packageDir, "html", mustJSONRaw(t, map[string]any{"displayMode": "natural"}))
	if err != nil {
...
func TestSaveNoteFaceFaceKindsCreatesMissingFaces(t *testing.T) {
	...
	mustExist(t, filepath.Join(base, "text.md"))
	mustExist(t, filepath.Join(base, "html-view.html"))
	faceDoc, err := svc.loadNoteFace("library", meta.Dir, "html")
	if err != nil {
		t.Fatal(err)
	}
...
func TestFaceKindsDoNotReorderExistingNote(t *testing.T) {
	...
	noteDir := filepath.Join(svc.libraryDir, notesDir, "2026-09", "face-order-existing")
...
func TestSaveHtmlFaceKeepsDisplayModeSettings(t *testing.T) {
	...
	packageDir := created.(map[string]any)["meta"].(noteMeta).Dir
	if _, err := svc.saveNoteFaceSettings("library", packageDir, "html", mustJSONRaw(t, map[string]any{"displayMode": "natural"})); err != nil {
		t.Fatalf("save displayMode failed: %v", err)
	}
