...
function log(message: string, extra?: unknown) {
  ...
  process.stderr.write(`[hypercortex-backend] ${message}${suffix}\n`)
}
...
async function handleFrame(ws: WebSocket, handler: DirectRequestHandler, frame: DirectClientRequest) {
...
export async function startDirectServer(options: {
  ...
  wss.on('connection', ws => {
    ws.on('message', data => {
      let frame: DirectClientRequest | null = null
      try {
