...
function endpointUrlWithToken(endpoint: any) {
  ...
  if (!url || !token) throw new Error('HyperCortex 后台 endpoint 不完整')
...
function waitForOpen(ws: WebSocket) {
  return new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('HyperCortex 后台连接超时')), OPEN_TIMEOUT_MS)
    ...
    ws.addEventListener('error', () => {
      clearTimeout(timer)
      reject(new Error('HyperCortex 后台连接失败'))
    }, { once: true })
  })
}
...
export async function createBackgroundClient(baseApi: any): Promise<BackgroundClient> {
  ...
  const client = new HyperCortexBackgroundClient(() => endpoint.call(baseApi.background))
  try {
    await client.open()
    return client
  } catch (error) {
...
class HyperCortexBackgroundClient implements BackgroundClient {
  ...
  invoke = async <T = unknown,>(method: string, params?: unknown): Promise<T> => {
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      this.markDisconnected(ws, new Error('HyperCortex 后台连接已关闭'))
      await this.ensureConnected()
    }
  ...
  close = () => {
    ...
    this.rejectPending(new Error('HyperCortex 后台连接已关闭'))
    const ws = this.ws
    this.ws = null
  ...
  private async ensureConnected(): Promise<void> {
    if (this.closed) throw new Error('HyperCortex 后台连接已关闭')
  ...
  private async connect(): Promise<void> {
    const ws = new WebSocket(endpointUrlWithToken(await this.loadEndpoint()))
    this.ws = ws
    ws.addEventListener('message', event => this.handleMessage(event))
    ws.addEventListener('close', () => this.markDisconnected(ws, new Error('HyperCortex 后台连接已关闭')))
    ws.addEventListener('error', () => this.markDisconnected(ws, new Error('HyperCortex 后台连接异常')))
    try {
  ...
  private async reconnect(force: boolean) {
    ...
    if (force && this.ws) {
      const ws = this.ws
      this.ws = null
      this.rejectPending(new Error('HyperCortex 后台连接刷新中'))
      if (ws.readyState !== WebSocket.CLOSED) ws.close()
    }
    await this.ensureConnected()
  }
