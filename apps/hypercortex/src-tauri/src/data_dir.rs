...
const CONFIG_FILE: &str = "hypercortex-settings.json";
const WRITE_TEST_FILE: &str = ".fw-hypercortex-write-test";
...
pub(crate) fn save_data_dir(app: &tauri::AppHandle, data_dir: &Path) -> Result<(), String> {
    ...
    let settings = HyperCortexSettings {
...
fn load_settings(app: &tauri::AppHandle) -> Result<HyperCortexSettings, String> {
    let path = settings_path(app)?;
    if !path.is_file() {
        return Ok(HyperCortexSettings::default());
    }
