...
pub(crate) const HYPERCORTEX_APP_ID: &str = "hypercortex";
...
pub(crate) fn session_token() -> String {
    ...
    getrandom::fill(&mut bytes).expect("failed to generate HyperCortex control token");
    format!("hc-{}", hex_token(&bytes))
}
        ...
        .spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!(
                            "[hypercortex-app] {} connection failed: {error}",
                            server_name
                        );
                        break;
                    }
