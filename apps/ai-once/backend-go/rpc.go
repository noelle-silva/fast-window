...
func (s *rpcServer) authorizeControlRequest(w http.ResponseWriter, r *http.Request) bool {
	if r.Header.Get("X-FW-Control-Token") != s.token {
...
func (s *rpcServer) handleControl(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		writeJSONError(w, http.StatusMethodNotAllowed, "控制入口只接受 POST")
		return
	}
	if !s.authorizeControlRequest(w, r) {
		return
	}
	...
	case controlActionInvokeCapability:
		...
		result, err = invokeCapability(r.Context(), s.svc, capabilityInvokeRequest{
			CapabilityID: body.CapabilityID,
			Input:        input,
			Config:       body.Config,
		})
	case controlActionQueryCapabilityOptions:
		result, err = queryCapabilityOptions(s.svc, capabilityQueryOptionsRequest{
			CapabilityID: body.CapabilityID,
