...
func (s *service) refreshModels(ctx context.Context, providerID string) (AppData, error) {
	...
	if res.StatusCode < 200 || res.StatusCode >= 300 {
		return data, fmt.Errorf("models HTTP %d: %s", res.StatusCode, string(body))
	}
	ids, err := parseModels(body)
	if err != nil {
		return data, err
	}
...
func parseModels(body []byte) ([]string, error) {
	...
	if list == nil {
		return nil, errors.New("models 响应格式不支持（期望 data[] 或 models[]）")
	}
...
func (s *service) ask(ctx context.Context, req AskRequest) (HistoryEntry, error) {
	...
	if pi < 0 {
		return HistoryEntry{}, errors.New("供应商不存在")
	}
	space, si := findSpace(data, req.SpaceID)
	if si < 0 {
		return HistoryEntry{}, errors.New("空间不存在")
	}
	tpl := findTemplate(space, firstNonEmpty(req.TemplateID, space.ActiveTemplateID))
	if tpl.ID == "" {
		return HistoryEntry{}, errors.New("模板不存在")
	}
	model := strings.TrimSpace(req.Model)
	if model == "" {
		model = strings.TrimSpace(space.DefaultModelByProvider[providerID])
	}
	if model == "" {
		return HistoryEntry{}, errors.New("请选择/填写模型")
	}
	input := strings.TrimSpace(req.Input)
	if input == "" && len(req.Images) == 0 {
		return HistoryEntry{}, errors.New("输入不能为空")
	}
	if err := validateImages(req.Images, data.Settings.ImageMaxCount, data.Settings.ImageMaxMB); err != nil {
		return HistoryEntry{}, err
	}
	p := data.Settings.Providers[pi]
	if err := requireProvider(p, true); err != nil {
		return HistoryEntry{}, err
	}
	...
	if err := s.recordHistory(data, &entry, req.Images); err != nil {
		return entry, err
	}
	if callErr != nil {
...
func buildMessages(systemPrompt, input string, images []DraftImage) []map[string]any {
	...
	if len(images) == 0 {
		return append(messages, map[string]any{"role": "user", "content": input})
	}
...
func callChatCompletion(ctx context.Context, p Provider, payload map[string]any, timeoutSettings TimeoutSettings) (string, error) {
	...
	if res.StatusCode < 200 || res.StatusCode >= 300 {
		return "", fmt.Errorf("AI HTTP %d: %s", res.StatusCode, errorFromBody(raw, body))
	}
	choices, _ := raw["choices"].([]any)
	if len(choices) == 0 {
		return "", errors.New("响应为空（choices 不存在）")
	}
...
func validateImages(images []DraftImage, maxCount int, maxMB float64) error {
	...
	if len(images) > maxCount {
		return fmt.Errorf("最多 %d 张图片", maxCount)
	}
	maxBytes := int64(maxMB * 1024 * 1024)
	for _, img := range images {
		if !strings.HasPrefix(img.Type, "image/") {
			return fmt.Errorf("%s 不是图片", firstNonEmpty(img.Name, "文件"))
		}
		if img.Size > maxBytes {
			return fmt.Errorf("%s 超过 %.1f MB", firstNonEmpty(img.Name, "图片"), maxMB)
		}
		if !strings.HasPrefix(img.DataURL, "data:image/") {
			return fmt.Errorf("%s 缺少合法 data URL", firstNonEmpty(img.Name, "图片"))
		}
		comma := strings.IndexByte(img.DataURL, ',')
		if comma < 0 {
			return fmt.Errorf("%s data URL 无效", firstNonEmpty(img.Name, "图片"))
		}
		if _, err := base64.StdEncoding.DecodeString(img.DataURL[comma+1:]); err != nil {
			return fmt.Errorf("%s data URL 不是合法 base64", firstNonEmpty(img.Name, "图片"))
		}
	}
	return nil
}
...
func requireProvider(p Provider, requireKey bool) error {
	...
	if requireKey && strings.TrimSpace(p.APIKey) == "" {
...
func providerIndex(data AppData, id string) int {
	for i, p := range data.Settings.Providers {
		if p.ID == id {
			return i
		}
	}
...
func findSpace(data AppData, id string) (Space, int) {
	for i, s := range data.Spaces {
		if s.ID == id {
			return s, i
		}
	}
	if len(data.Spaces) > 0 && id == "" {
		return data.Spaces[0], 0
	}
	return Space{}, -1
}
func findTemplate(space Space, id string) Template {
	for _, t := range space.Templates {
		if t.ID == id {
			return t
		}
	}
	if len(space.Templates) > 0 && id == "" {
		return space.Templates[0]
	}
	return Template{}
}
func errorFromBody(raw map[string]any, body []byte) string {
	if e, _ := raw["error"].(map[string]any); e != nil {
