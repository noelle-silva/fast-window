...
pub(crate) fn start_single_instance_server(
    ...
    let token = random_token("ai-once-single");
    let endpoint = start_control_server(
        app,
        window_state,
        ControlServerConfig {
            name: "fw-ai-once-single-instance",
            app_id: AI_ONCE_APP_ID,
...
fn write_instance_state(
    ...
    let state = InstanceState {
        app_id: AI_ONCE_APP_ID.to_string(),
...
fn start_instance_state_refresh(endpoint: ControlEndpoint, desktop_identifier: String) {
    let _ = thread::Builder::new()
        .name("fw-ai-once-single-instance-state".to_string())
        .spawn(move || loop {
            ...
            if let Err(error) = write_instance_state(&endpoint, &desktop_identifier) {
                eprintln!("[ai-once] 刷新单实例状态失败: {error}");
            }
        });
}
...
mod tests {
    ...
    use crate::control_server::AI_ONCE_APP_ID;

    fn valid_state() -> InstanceState {
        InstanceState {
            app_id: AI_ONCE_APP_ID.to_string(),
            desktop_identifier: "com.fastwindow.aionce.dev".to_string(),
            server_id: SINGLE_INSTANCE_SERVER_ID.to_string(),
    ...
    fn validates_identifier_specific_state() {
        assert!(is_valid_instance_state(
            &valid_state(),
            "com.fastwindow.aionce.dev"
        ));
        assert!(!is_valid_instance_state(
            &valid_state(),
            "com.fastwindow.aionce"
        ));
    }
