...
use control_server::{
    available_commands, random_token, start_control_server, ControlServerConfig, AI_ONCE_APP_ID,
};
...
async fn pick_data_dir(
    ...
    let Some(path) = native_dialog::run_file_dialog(&app, |dialog| {
        dialog.set_title("选择 AI Once 数据目录").pick_folder()
    })?
    else {
        return Ok(None);
    };
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            start_control_server(
                ...
                window_state_setup.clone(),
                ControlServerConfig {
                    name: "fw-ai-once-control",
                    app_id: AI_ONCE_APP_ID,
                    server_id: "fw-control",
                    bind_addr: "127.0.0.1:0",
                    token: random_token("ai-once-control"),
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) = start_backend(handle, state).await {
                    ...
                    eprintln!("[ai-once] {error}");
                }
            });
            Ok(())
        })
        .build(context)
        .expect("error while building AI Once app")
        .run(move |app, event| {
