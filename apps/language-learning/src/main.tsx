...
function App() {
  ...
  const connect = React.useCallback(async (options?: { restartBackend?: boolean }) => {
    ...
    try {
      ...
      const [nextSettings, nextHealth] = await Promise.all([
        nextClient.request<AppSettings>('languageLearning.settings.get'),
        nextClient.request<Record<string, unknown>>('languageLearning.health'),
      ])
      ...
      setPhase('failed')
      setError(errorMessage(e, '启动 Language Learning 后台失败'))
      await refreshStatus()
    } finally {
      setBusy(false)
    }
  ...
  const saveSettings = React.useCallback(async () => {
    ...
    setError(null)
    try {
      const saved = await client.request<AppSettings>('languageLearning.settings.save', { message })
      setSettings(saved)
    } catch (e) {
