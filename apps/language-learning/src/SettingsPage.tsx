...
import type { AppSettings, DataDirStatus, DirectClient, FwLaunchInfo } from './types'
...
export function SettingsPage(props: SettingsPageProps) {
  ...
  const renderPanel = (tab: SettingsTab, content: React.ReactNode) => (
    <div
      ...
      id={`ll-settings-panel-${tab}`}
      aria-labelledby={`ll-settings-tab-${tab}`}
      className="ll-settings-panel"
    >
  ...
  return (
    <section className="ll-settings-page" aria-label="Language Learning 设置">
      <SettingsTabs items={SETTINGS_TABS} value={activeTab} onChange={setActiveTab} ariaLabel="设置分类" />

      {renderPanel('overview', (
        <div className="ll-settings-stack">
      ...
      {renderPanel('data', (
        <div className="ll-settings-stack">
          <article className="ll-panel">
            ...
            {status?.error ? <p className="ll-error-text">{status.error}</p> : null}
      ...
      {renderPanel('backend', (
        <div className="ll-settings-stack">
          <article className="ll-panel">
            <h2>后台健康</h2>
            <div className={`ll-status-badge ll-status-${phase}`}>{phase === 'ready' ? 'Backend Ready' : phase === 'failed' ? 'Needs Attention' : 'Starting'}</div>
      ...
      {renderPanel('example', (
        <div className="ll-settings-stack">
          <article className="ll-panel">
            ...
            <p className="ll-muted">最后保存：{settings?.updatedAt || '尚未保存'}</p>
          </article>
        </div>
      ))}
      ...
      {renderPanel('components', (
        <div className="ll-settings-stack">
          <article className="ll-panel">
            <h2>自绘组件</h2>
            <p className="ll-muted">无轨道滑动条：只绘制可拖动游标，不绘制传统轨道。</p>
            <TracklessSlider
              label="示范强度"
              min={0}
              max={100}
      ...
      {error ? <div className="ll-error-card" role="alert">{error}</div> : null}
    </section>
  )
}
