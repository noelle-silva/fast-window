...
function WindowControls({ actions }: { actions: WindowControlActions }) {
  return (
    <div className="ll-window-controls" data-window-controls="true" aria-label="窗口控制">
      ...
      <button type="button" className="ll-close-button" aria-label="关闭到托盘" onClick={() => run(actions.closeToTray)}>
        <span aria-hidden="true">×</span>
      </button>
    </div>
  )
...
export function AppTopbar(props: AppTopbarProps) {
  ...
  return (
    <header className="ll-topbar" onPointerDown={onPointerDown}>
      {page === 'settings' ? (
        <button type="button" className="ll-icon-button" onClick={onBack} aria-label="返回主页">
          <IconBack />
        </button>
      ) : null}

      <div className="ll-topbar-title" aria-live="polite">{page === 'settings' ? '设置' : ''}</div>
      <div className="ll-topbar-spacer" />

      {page === 'home' ? (
        <button type="button" className="ll-icon-button" onClick={onOpenSettings} aria-label="设置">
          <IconSettings />
        </button>
      ) : null}
