...
func run() error {
	...
	if token == "" {
		return errors.New("language-learning backend missing FW_APP_SESSION_TOKEN")
	}
...
func newService() (*service, error) {
	...
	if dataDir == "" {
		return nil, errors.New("language-learning backend missing FW_APP_DATA_DIR")
	}
...
func (svc *service) dispatch(method string, params json.RawMessage) (any, error) {
	...
	case "languageLearning.settings.save":
		...
		return svc.saveSettings(payload.Message)
	default:
...
func (svc *service) ensureReady() error {
	...
	if _, err := os.Stat(filepath.Join(svc.dataDir, settingsFile)); errors.Is(err, os.ErrNotExist) {
		_, err = svc.saveSettings("Hello from language-learning")
		return err
	}
	return nil
}
...
func (svc *service) saveSettings(message string) (appSettings, error) {
	settings := appSettings{SchemaVersion: dataSchemaVersion, DataVersion: dataVersion, Message: strings.TrimSpace(message), UpdatedAt: nowText()}
	if settings.Message == "" {
		settings.Message = "Hello from language-learning"
	}
...
func ensureWritable(dir string) error {
	path := filepath.Join(dir, ".fw-language-learning-write-test")
