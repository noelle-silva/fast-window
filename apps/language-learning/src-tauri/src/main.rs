...
async fn pick_data_dir(
    ...
    let Some(path) = native_dialog::run_file_dialog(&app, |dialog| {
        dialog
            .set_title("选择 Language Learning 数据目录")
            .pick_folder()
    })?
    else {
        return Ok(None);
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            start_control_server(
                ...
                window_state_setup.clone(),
                ControlServerConfig {
                    name: "fw-language-learning-control",
                    app_id: LANGUAGE_LEARNING_APP_ID,
                    server_id: "fw-control",
                    bind_addr: "127.0.0.1:0",
                    token: random_token("ll-control"),
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) = start_backend(handle, state).await {
                    ...
                    eprintln!("[language-learning] {error}");
                }
            });
            Ok(())
        })
        .build(context)
        .expect("error while building Language Learning app")
        .run(move |app, event| {
