...
pub(crate) const LANGUAGE_LEARNING_APP_ID: &str = "language-learning";
...
pub(crate) fn available_commands() -> Vec<AppCommandDescriptor> {
    vec![
        AppCommandDescriptor {
            id: "open-language-learning",
            title: "打开 Language Learning",
        },
        AppCommandDescriptor {
            id: "show-health",
            title: "查看 Language Learning 健康状态",
        },
        AppCommandDescriptor {
            id: "edit-settings",
            title: "打开 Language Learning 设置",
        },
    ]
}
...
pub(crate) fn random_token(prefix: &str) -> String {
    ...
    getrandom::fill(&mut bytes).expect("failed to generate language-learning token");
        ...
        .spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!(
                            "[language-learning] {} connection failed: {error}",
                            server_name
                        );
                        break;
                    }
