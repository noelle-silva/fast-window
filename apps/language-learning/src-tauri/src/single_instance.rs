...
pub(crate) fn start_single_instance_server(
    ...
    let token = random_token("ll-single");
    let endpoint = start_control_server(
        app,
        window_state,
        ControlServerConfig {
            name: "fw-language-learning-single-instance",
            app_id: LANGUAGE_LEARNING_APP_ID,
...
fn write_instance_state(
    ...
    let state = InstanceState {
        app_id: LANGUAGE_LEARNING_APP_ID.to_string(),
...
fn is_valid_instance_state(state: &InstanceState, desktop_identifier: &str) -> bool {
    state.app_id == LANGUAGE_LEARNING_APP_ID
...
mod tests {
    ...
    use crate::control_server::LANGUAGE_LEARNING_APP_ID;

    fn valid_state() -> InstanceState {
        InstanceState {
            app_id: LANGUAGE_LEARNING_APP_ID.to_string(),
            desktop_identifier: "com.fastwindow.languagelearning.dev".to_string(),
            server_id: SINGLE_INSTANCE_SERVER_ID.to_string(),
    ...
    fn validates_identifier_specific_state() {
        assert!(is_valid_instance_state(
            &valid_state(),
            "com.fastwindow.languagelearning.dev"
        ));
        assert!(!is_valid_instance_state(
            &valid_state(),
            "com.fastwindow.languagelearning"
        ));
    }
