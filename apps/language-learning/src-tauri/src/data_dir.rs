...
const CONFIG_FILE: &str = "language-learning-settings.json";
const WRITE_TEST_FILE: &str = ".fw-language-learning-write-test";
...
#[serde(rename_all = "camelCase")]
struct LanguageLearningSettings {
...
pub(crate) fn save_data_dir(app: &tauri::AppHandle, data_dir: &Path) -> Result<(), String> {
    ...
    let settings = LanguageLearningSettings {
...
fn load_settings(app: &tauri::AppHandle) -> Result<LanguageLearningSettings, String> {
    let path = settings_path(app)?;
    if !path.is_file() {
        return Ok(LanguageLearningSettings::default());
    }
