...
fn write_instance_state(endpoint: &ControlEndpoint, desktop_identifier: &str) -> Result<(), String> {
...
mod tests {
    ...
    fn valid_state() -> InstanceState {
        InstanceState {
            app_id: AI_DRAW_APP_ID.to_string(),
            desktop_identifier: "com.fastwindow.aidraw.dev".to_string(),
            server_id: SINGLE_INSTANCE_SERVER_ID.to_string(),
    ...
    fn validates_dynamic_instance_state_identity() {
        ...
        assert!(!is_valid_instance_state(&wrong_app, "com.fastwindow.aidraw.dev"));

        let mut wrong_server = valid_state();
        wrong_server.server_id = "fw-control".to_string();
        assert!(!is_valid_instance_state(&wrong_server, "com.fastwindow.aidraw.dev"));

        let mut wrong_addr = valid_state();
        wrong_addr.addr = "0.0.0.0:49152".to_string();
        assert!(!is_valid_instance_state(&wrong_addr, "com.fastwindow.aidraw.dev"));

        let mut external_addr = valid_state();
        external_addr.addr = "192.168.1.10:49152".to_string();
        assert!(!is_valid_instance_state(&external_addr, "com.fastwindow.aidraw.dev"));

        let mut zero_port = valid_state();
        zero_port.addr = "127.0.0.1:0".to_string();
        assert!(!is_valid_instance_state(&zero_port, "com.fastwindow.aidraw.dev"));

        let mut malformed_addr = valid_state();
        malformed_addr.addr = "not-an-addr".to_string();
        assert!(!is_valid_instance_state(&malformed_addr, "com.fastwindow.aidraw.dev"));

        let mut empty_token = valid_state();
        empty_token.token = " ".to_string();
        assert!(!is_valid_instance_state(&empty_token, "com.fastwindow.aidraw.dev"));

        let mut zero_pid = valid_state();
        zero_pid.pid = 0;
        assert!(!is_valid_instance_state(&zero_pid, "com.fastwindow.aidraw.dev"));

        let mut stale = valid_state();
        stale.updated_at = 1;
        assert!(!is_valid_instance_state(&stale, "com.fastwindow.aidraw.dev"));
    }
