...
pub(crate) const AI_DRAW_APP_ID: &str = "ai-draw";
        ...
        .spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!("[ai-draw-app] {} connection failed: {error}", server_name);
                        break;
                    }
                }
            }
...
mod tests {
    ...
    fn accepts_matching_control_response_identity() {
        let body =
            r#"{"ok":true,"appId":"ai-draw","serverId":"single-instance","protocolVersion":1}"#;
        assert!(control_response_matches(body, "ai-draw", "single-instance"));
    }
