...
export async function createAiDrawDirectGateway(options: AiDrawDirectGatewayOptions): Promise<AiDrawGateway> {
  ...
  return {
    ...
    outputImages: {
      ...
      pickOutputDir: async () => {
        if (typeof hostApi.pickOutputDir !== 'function') return null
        const outputDir = await hostApi.pickOutputDir()
      ...
      openOutputDir: async () => {
        ...
        const outputDir = (await direct.invoke<{ outputDir: string }>(AI_DRAW_DIRECT_METHOD.outputImagesGetOutputDir, {})).outputDir
        await hostApi.openOutputDir(outputDir)
      },
