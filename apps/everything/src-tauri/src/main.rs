...
use control_server::{
    available_commands, random_token, start_control_server, ControlServerConfig, EVERYTHING_APP_ID,
};
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            start_control_server(
                ...
                window_state_setup.clone(),
                ControlServerConfig {
                    name: "fw-everything-control",
                    app_id: EVERYTHING_APP_ID,
                    server_id: "fw-control",
                    bind_addr: "127.0.0.1:0",
                    token: random_token("everything-control"),
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) = start_backend(handle, state).await {
                    ...
                    eprintln!("[everything] {error}");
                }
            });
            Ok(())
        })
        .build(context)
        .expect("error while building Everything app")
        .run(move |app, event| {
