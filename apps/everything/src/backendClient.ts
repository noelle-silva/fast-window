...
const REQUEST_TIMEOUTS_BY_METHOD: Record<string, number> = {
  'everything.search': SEARCH_REQUEST_TIMEOUT_MS,
  'everything.setup.enableGlobal': LONG_REQUEST_TIMEOUT_MS,
  'everything.runtime.restart': LONG_REQUEST_TIMEOUT_MS,
}
...
function requestTimeoutMs(method: string, options?: RequestOptions) {
  const explicit = Number(options?.timeoutMs || 0)
  if (Number.isFinite(explicit) && explicit > 0) return explicit
  return REQUEST_TIMEOUTS_BY_METHOD[method] || DEFAULT_REQUEST_TIMEOUT_MS
}
...
class EverythingDirectClient implements DirectClient {
  ...
  request = async <T,>(method: string, params?: unknown, options?: RequestOptions): Promise<T> => {
    ...
    const id = `everything-${Date.now()}-${++this.seq}`
    const timeoutMs = requestTimeoutMs(method, options)
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id)
        reject(new Error(`${method} 后台请求超时`))
      }, timeoutMs)
...
export async function createDirectClient(): Promise<DirectClient> {
  const client = new EverythingDirectClient()
  try {
    await client.open()
    return client
  } catch (error) {
