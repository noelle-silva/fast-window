...
export function SettingsTabs<T extends string>(props: SettingsTabsProps<T>) {
  ...
  const focusTab = React.useCallback((index: number) => {
    ...
    requestAnimationFrame(() => {
      document.getElementById(`everything-settings-tab-${item.id}`)?.focus()
    })
  }, [items, onChange])
  ...
  return (
    <div className="everything-settings-toolbar">
      <div className="everything-settings-tabs" role="tablist" aria-label={ariaLabel} onKeyDown={onKeyDown}>
        {items.map(item => {
          ...
          return (
            <button
              key={item.id}
              type="button"
              className="everything-settings-tab"
              role="tab"
              id={`everything-settings-tab-${item.id}`}
              aria-selected={selected}
              aria-controls={`everything-settings-panel-${item.id}`}
