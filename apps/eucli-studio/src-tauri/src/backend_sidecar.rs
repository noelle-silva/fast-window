...
impl BackendState {
    ...
    pub(crate) async fn endpoint(&self) -> Result<BackendEndpoint, String> {
        self.process.endpoint("eucli-studio 后台未就绪").await
    }
}
...
pub(crate) async fn start_backend(
    ...
    let session_token = random_token("eucli-backend");
...
fn spawn_backend_child(
    ...
    cmd.env("EUCLI_STUDIO_RELEASE_JSON", RELEASE_JSON);
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| format!("启动 eucli-studio Go 后台失败: {e}"))?;
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| "eucli-studio Go 后台 stdout 不可用".to_string())?;
    Ok((child, stdout))
}
...
fn backend_endpoint_from_ready(
    ...
    if ipc.get("mode").and_then(|v| v.as_str()) != Some("direct") {
        return None;
    }
    if ipc.get("transport").and_then(|v| v.as_str()) != Some("local-websocket") {
        return None;
    }
    let protocol_version = ipc
        .get("protocolVersion")
        .and_then(|v| v.as_u64())
...
fn resolve_backend_sidecar(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    ...
    Err(format!("eucli-studio Go 后台 sidecar 不存在: {exe_name}"))
}
