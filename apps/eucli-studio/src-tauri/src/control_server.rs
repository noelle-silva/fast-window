...
pub(crate) const EUCLI_STUDIO_APP_ID: &str = "eucli-studio";
...
pub(crate) fn random_token(prefix: &str) -> String {
    ...
    getrandom::fill(&mut bytes).expect("failed to generate eucli-studio token");
        ...
        .spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!("[eucli-studio] {} connection failed: {error}", server_name);
                        break;
                    }
                }
            }
