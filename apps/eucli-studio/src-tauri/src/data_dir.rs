...
pub(crate) fn save_data_dir(app: &tauri::AppHandle, data_dir: &Path) -> Result<(), String> {
    ...
    let settings = EucliStudioSettings {
...
fn load_settings(app: &tauri::AppHandle) -> Result<EucliStudioSettings, String> {
    let path = settings_path(app)?;
    if !path.is_file() {
        return Ok(EucliStudioSettings::default());
    }
