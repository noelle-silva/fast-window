...
pub(crate) fn start_single_instance_server(
    ...
    let token = random_token("eucli-single");
    let endpoint = start_control_server(
        app,
        window_state,
        ControlServerConfig {
            name: "fw-eucli-studio-single-instance",
...
fn write_instance_state(
    ...
    let state = InstanceState {
        app_id: EUCLI_STUDIO_APP_ID.to_string(),
...
fn is_valid_instance_state(state: &InstanceState, desktop_identifier: &str) -> bool {
    state.app_id == EUCLI_STUDIO_APP_ID
...
mod tests {
    ...
    use crate::control_server::EUCLI_STUDIO_APP_ID;

    fn valid_state() -> InstanceState {
        InstanceState {
            app_id: EUCLI_STUDIO_APP_ID.to_string(),
            desktop_identifier: "com.fastwindow.euclistudio.dev".to_string(),
            server_id: SINGLE_INSTANCE_SERVER_ID.to_string(),
