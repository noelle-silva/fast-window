...
#[tauri::command]
fn exit_app(app: tauri::AppHandle, state: tauri::State<'_, ShutdownState>) {
    state.shutdown(app);
}
...
fn desktop_identifier() -> &'static str {
    ...
    {
        "com.fastwindow.euclistudio.dev"
    }
    #[cfg(not(debug_assertions))]
    {
        "com.fastwindow.euclistudio"
    }
}
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            start_control_server(
                ...
                window_state_setup.clone(),
                ControlServerConfig {
                    name: "fw-eucli-studio-control",
                    app_id: EUCLI_STUDIO_APP_ID,
                    server_id: "fw-control",
                    bind_addr: "127.0.0.1:0",
                    token: random_token("eucli-control"),
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) = start_backend(handle, state).await {
                    ...
                    eprintln!("[eucli-studio] {error}");
                }
            });
            Ok(())
        })
        .build(context)
        .expect("error while building eucli-studio app")
        .run(move |app, event| {
