...
export function ConfirmDialog(props: { open: boolean; controller: any; draft: any; roles: any[]; groups: any[]; providers: any[]; workspaces?: any[] }) {
  ...
  const wid = String((draft as any)?.deleteWorkspaceId || '')
  ...
  const workspace = wid ? (Array.isArray(workspaces) ? workspaces : []).find((w) => String(w?.id || '') === wid) : null
