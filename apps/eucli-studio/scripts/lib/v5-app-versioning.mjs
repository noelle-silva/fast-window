...
function makeCargoTomlTarget(root, filePath) {
  const label = relLabel(root, filePath)
  return {
    label,
    filePath,
...
function makeCargoLockTarget(root, filePath, packageName) {
  const label = relLabel(root, filePath)
  return {
    label,
    filePath,
...
export async function createV5AppVersionPlan(appDirArg = rootDir) {
  ...
  // eucli-studio 客户端发布资料：Rust 壳将它编译进二进制传给 Go 后台，版本必须与产品版本保持一体。
  const releaseJsonPath = path.join(appDir, 'release.json')

  const cargoLabel = relLabel(appDir, cargoTomlPath)
  ...
  return {
    ...
    targets: [
      makeJsonTarget(appDir, packageJsonPath),
      makeJsonTarget(appDir, tauriConfPath),
      makeJsonTarget(appDir, tauriDevConfPath),
      makeCargoTomlTarget(appDir, cargoTomlPath),
      makeCargoLockTarget(appDir, cargoLockPath, cargoPackage.name),
      makeJsonTarget(appDir, releaseJsonPath),
    ],
  }
}
