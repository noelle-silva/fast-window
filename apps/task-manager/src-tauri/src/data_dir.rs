...
const CONFIG_FILE: &str = "task-manager-settings.json";
const WRITE_TEST_FILE: &str = ".fw-task-manager-write-test";
...
pub(crate) fn save_data_dir(app: &tauri::AppHandle, data_dir: &Path) -> Result<(), String> {
    ...
    let settings = TaskManagerSettings {
...
fn load_settings(app: &tauri::AppHandle) -> Result<TaskManagerSettings, String> {
    let path = settings_path(app)?;
    if !path.is_file() {
        return Ok(TaskManagerSettings::default());
    }
