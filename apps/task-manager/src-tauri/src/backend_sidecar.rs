...
impl BackendState {
    ...
    pub(crate) async fn endpoint(&self) -> Result<BackendEndpoint, String> {
        self.process.endpoint("Task Manager 后台未就绪").await
    }
}
...
pub(crate) async fn start_backend(
    ...
    let session_token = random_token("tasks-backend");
...
fn spawn_backend_child(
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| format!("启动 Task Manager 后台失败: {e}"))?;
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| "Task Manager 后台 stdout 不可用".to_string())?;
    Ok((child, stdout))
}
...
fn resolve_backend_sidecar(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    ...
    Err(format!("Task Manager 后台 sidecar 不存在: {exe_name}"))
}
