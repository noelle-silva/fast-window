...
pub(crate) const TASK_MANAGER_APP_ID: &str = "task-manager";
...
pub(crate) fn random_token(prefix: &str) -> String {
    ...
    getrandom::fill(&mut bytes).expect("failed to generate task manager token");
        ...
        .spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!(
                            "[task-manager] {} connection failed: {error}",
                            server_name
                        );
                        break;
                    }
