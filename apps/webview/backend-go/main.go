...
func run() error {
	...
	if token == "" {
		return errors.New("webview backend missing FW_APP_SESSION_TOKEN")
	}
...
func newService() (*service, error) {
	...
	if dataDir == "" {
		return nil, errors.New("webview backend missing FW_APP_DATA_DIR")
	}
...
func (svc *service) ensureReady() error {
	...
	if _, err := os.Stat(filepath.Join(svc.dataDir, settingsFile)); errors.Is(err, os.ErrNotExist) {
		_, err = svc.saveSettings("Hello from webview")
		return err
	}
	return nil
}
...
func (svc *service) saveSettings(message string) (appSettings, error) {
	...
	if settings.Message == "" {
		settings.Message = "Hello from webview"
	}
