...
import { applyContainerItemDesktopExtraction, isContainerSoftClosedForExtractDrag, resolveContainerExtractDragMode, resolveContainerExtractNextDragMode } from '../src/collections/containerExtractDragState.ts'
import { resolveContainerDropSurface } from '../src/collections/containerDropResolution.ts'
import { isContainerDropTargetActive, resolveDesktopDragMode, resolveDesktopDropIntent } from '../src/collections/desktopDragState.ts'
import { desktopEntryKey } from '../src/collections/folder-grid/desktopEntries.ts'
import { projectExternalItemDrag } from '../src/collections/folder-grid/desktopDragProjection.ts'
...
function desktopItemEntry(id, layout) {
  return {
    ...itemEntry(id),
    layout,
  }
...
function collectionsDoc() {
  return {
    schemaVersion: 1,
    dataVersion: 1,
    groups: [{ id: 'default', name: '默认' }],
    items: [
      { ...collectionItem('inside'), containerId: 'container-a', containerLayout: { x: 0, y: 0 } },
      { ...collectionItem('desktop'), layout: { x: 1, y: 0 } },
    ],
...
describe('desktop drag drop intent', () => {
  ...
  it('resolves a new-container drop from an overlay item hover target', () => {
    const intent = resolveDesktopDropIntent({
      dragMode: 'overlay',
      hoverTarget: { entry: itemEntry('item-b'), layout: { x: 3, y: 4 } },
    }, null, null)

    assert.deepEqual(intent, { kind: 'new-container', targetItemId: 'item-b', layout: { x: 3, y: 4 } })
  })
})
...
describe('container extract drag state', () => {
  ...
  it('soft closes only the active container during desktop extraction', () => {
    const drag = { containerId: 'container-a', item: collectionItem('inside'), mode: 'desktop' }
  ...
  it('applies desktop extraction without mutating unrelated container items', () => {
    const nextDoc = applyContainerItemDesktopExtraction(collectionsDoc(), 'container-a', 'inside', [
  ...
  it('rejects extraction patches for items that are still in a container', () => {
    assert.throws(() => applyContainerItemDesktopExtraction(collectionsDoc(), 'container-a', 'inside', [
      { kind: 'item', id: 'inside', layout: { x: 2, y: 0 } },
      { kind: 'item', id: 'missing', layout: { x: 3, y: 0 } },
    ]), /desktop entry not found/)
    assert.throws(() => applyContainerItemDesktopExtraction(collectionsDoc(), 'container-a', 'desktop', [
...
describe('container extract desktop projection', () => {
  ...
  const baseEntries = [
    ...
    desktopItemEntry('desktop-a', { x: 1, y: 0 }),
  ]
