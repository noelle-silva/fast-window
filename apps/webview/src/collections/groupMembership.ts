import type { CollectionItem, Workspace } from './types'
...
export function groupItemCount(workspace: Workspace, groupId: string): number {
  return workspace.items.filter(item => itemHasGroup(item, groupId)).length
}

export function groupContainerCount(workspace: Workspace, groupId: string): number {
