...
import { IdentityManagementSection } from './IdentityManagementSection'
...
import type { DataDirStatus, DesktopIconLayout, DesktopWallpaperView, IdentityInfo, WorkspaceView } from './types'
...
export function SettingsDialog(props: {
  ...
  onClose(): void
  onDeleteIdentity(spaceId: string): void
      ...
      <DialogContent sx={{ p: 3 }}>
        <Stack spacing={2.25}>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>管理桌面图标、壁纸、视频倍速、独立空间登录信息与数据目录。</Typography>
          </Box>
          <Tabs value={tab} onChange={(_, value: SettingsTab) => setTab(value)} variant="fullWidth">
            <Tab value="general" label="常规" />
            <Tab value="identities" label="独立空间登录信息" />
          </Tabs>
          {tab === 'general' ? (
            <>
            ...
            <IdentityManagementSection
              ...
              onDelete={props.onDeleteIdentity}
