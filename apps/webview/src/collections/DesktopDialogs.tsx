...
import type { CollectionContainer, ContainerFormState, DesktopIcon, IconAppearanceCandidate, IconAppearanceState, WebIconDiscoveryProgress } from './types'
...
export function IconAppearancePanel(props: {
  ...
  const iconFetchLabel = hasWebIcons ? '刷新网页图标' : '获取网页图标'
  const fetchDisabled = props.busy || props.webIconDiscovery.active
      ...
      <Stack spacing={1.75}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '132px 1fr' }, gap: 2, alignItems: 'stretch' }}>
          <Box sx={theme => ({ borderRadius: 4, p: 2, display: 'grid', placeItems: 'center', bgcolor: alpha(theme.palette.primary.main, 0.07), border: `1px solid ${alpha(theme.palette.primary.main, 0.14)}` })}>
            <IconPreview assetUrl={props.assetUrl} dataUrl={draftDataUrl} icon={draftIcon || defaultDesktopIcon} seed={props.seed || 'icon'} size={92} radius={28} />
          </Box>
          <Stack spacing={1.25}>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
              <Button startIcon={props.webIconDiscovery.active ? <CircularProgress size={16} color="inherit" /> : <TravelExploreRoundedIcon />} onClick={props.onFetchWebIcons} disabled={fetchDisabled}>{iconFetchLabel}</Button>
    ...
    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(86px, 1fr))', gap: 1 }}>
      {props.candidates.map(candidate => {
        ...
        return (
          ...
          >
            <IconPreview assetUrl={props.assetUrl} dataUrl={candidate.dataUrl} icon={candidate.icon || defaultDesktopIcon} seed={`${props.seed}-${candidate.id}`} size={54} radius={16} shadow={false} />
...
function IconPreview(props: {
  ...
  return <DesktopIconVisual assetUrl={props.assetUrl} icon={props.icon} seed={props.seed} size={props.size} radius={props.radius} shadow={props.shadow} />
}
