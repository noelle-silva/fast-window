...
function WindowControls({ actions }: { actions: WindowControlActions }) {
  return (
    <div className="reference-window-controls" data-window-controls="true" aria-label="窗口控制">
      ...
      <button type="button" className="reference-close-button" aria-label="关闭到托盘" onClick={() => run(actions.closeToTray)}>
        <span aria-hidden="true">×</span>
      </button>
    </div>
  )
...
export function ReferenceTopbar(props: ReferenceTopbarProps) {
  ...
  return (
    <header className="reference-topbar" onPointerDown={onPointerDown}>
      {page === 'settings' ? (
        <button type="button" className="reference-icon-button" onClick={onBack} aria-label="返回主页">
          <IconBack />
        </button>
      ) : null}

      <div className="reference-topbar-title" aria-live="polite">{page === 'settings' ? '倍速设置' : 'webview'}</div>
      <div className="reference-topbar-spacer" />

      {page === 'home' ? (
        <button type="button" className="reference-icon-button" onClick={onOpenSettings} aria-label="设置">
          <IconSettings />
        </button>
      ) : null}
