...
use crate::control_server::{
    ...
    ControlServerConfig, AI_STUDIO_APP_ID,
};
...
fn instance_state_dir() -> PathBuf {
    if cfg!(target_os = "windows") {
        if let Some(dir) = env::var_os("LOCALAPPDATA").or_else(|| env::var_os("APPDATA")) {
            return PathBuf::from(dir).join("FastWindow").join("AI Studio");
        }
    }

    if let Some(dir) = env::var_os("XDG_RUNTIME_DIR") {
        return PathBuf::from(dir).join("fast-window").join("ai-studio");
    }
    ...
    env::temp_dir().join("fast-window-ai-studio")
}
