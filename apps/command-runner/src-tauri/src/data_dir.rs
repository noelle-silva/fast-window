...
const CONFIG_FILE: &str = "command-runner-settings.json";
const WRITE_TEST_FILE: &str = ".fw-command-runner-write-test";
...
fn load_settings(app: &tauri::AppHandle) -> Result<CommandRunnerSettings, String> {
    let path = settings_path(app)?;
    if !path.is_file() {
        return Ok(CommandRunnerSettings::default());
    }
