...
use control_server::{
    available_commands, random_token, start_control_server, ControlServerConfig,
    COMMAND_RUNNER_APP_ID,
};
...
async fn pick_data_dir(
    ...
    let Some(path) = native_dialog::run_file_dialog(&app, |dialog| {
        dialog
            .set_title("选择 Command Runner 数据目录")
            .pick_folder()
    })?
    else {
        return Ok(None);
...
#[tauri::command]
fn pick_executable(app: tauri::AppHandle, title: String) -> Result<Option<String>, String> {
    let Some(path) = native_dialog::run_file_dialog(&app, |dialog| {
        dialog
            .set_title(&title)
            .add_filter("可执行程序", &["exe"])
            .pick_file()
    })?
    else {
        return Ok(None);
...
fn desktop_identifier() -> &'static str {
    ...
    {
        "com.fastwindow.commandrunner.dev"
    }
    #[cfg(not(debug_assertions))]
    {
        "com.fastwindow.commandrunner"
    }
}
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            start_control_server(
                ...
                window_state_setup.clone(),
                ControlServerConfig {
                    name: "fw-command-runner-control",
                    app_id: COMMAND_RUNNER_APP_ID,
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) = start_backend(handle, state).await {
                    ...
                    eprintln!("[command-runner] {error}");
                }
            });
            Ok(())
        })
        .build(context)
        .expect("error while building Command Runner app")
        .run(move |app, event| {
