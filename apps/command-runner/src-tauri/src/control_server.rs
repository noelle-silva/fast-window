...
pub(crate) const COMMAND_RUNNER_APP_ID: &str = "command-runner";
...
pub(crate) fn available_commands() -> Vec<AppCommandDescriptor> {
    vec![
        AppCommandDescriptor {
            id: "open-command-runner",
            title: "打开 Command Runner",
        },
        AppCommandDescriptor {
            id: "new-repo",
            title: "注册新仓库",
...
pub(crate) fn random_token(prefix: &str) -> String {
    ...
    getrandom::fill(&mut bytes).expect("failed to generate command runner app token");
        ...
        .spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!(
                            "[command-runner] {} connection failed: {error}",
                            server_name
                        );
                        break;
                    }
