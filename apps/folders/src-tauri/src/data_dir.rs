...
const CONFIG_FILE: &str = "folders-settings.json";
const WRITE_TEST_FILE: &str = ".fw-folders-write-test";
...
fn load_settings(app: &tauri::AppHandle) -> Result<FoldersSettings, String> {
    let path = settings_path(app)?;
    if !path.is_file() {
        return Ok(FoldersSettings::default());
    }
