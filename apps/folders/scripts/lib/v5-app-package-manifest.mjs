...
const protocolDir = path.join(rootDir, protocolDirName)
export const V5_APP_MANIFEST_FILE = 'fw-app.json'
export const V5_APP_MANIFEST_PATH = path.join(protocolDir, V5_APP_MANIFEST_FILE)
...
export function normalizeV5AppManifest(raw, { appDir, expectedId, manifestPath }) {
  ...
  if (id !== expectedId) throw new Error(`发布声明 id 与预期不一致: manifest=${id}, expected=${expectedId}`)
...
export async function loadAppConfig() {
  ...
  const manifestPath = V5_APP_MANIFEST_PATH
  ...
  const normalizedManifest = normalizeV5AppManifest(manifest, { appDir, expectedId: manifest.id, manifestPath })
