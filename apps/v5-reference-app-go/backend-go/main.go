...
func run() error {
	...
	if token == "" {
		return errors.New("v5-reference-app-go backend missing FW_APP_SESSION_TOKEN")
	}
...
func newService() (*service, error) {
	...
	if dataDir == "" {
		return nil, errors.New("v5-reference-app-go backend missing FW_APP_DATA_DIR")
	}
...
func (svc *service) dispatch(method string, params json.RawMessage) (any, error) {
	...
	case "reference.settings.get":
		return svc.readSettings()
	case "reference.settings.save":
		var payload struct {
			Message string `json:"message"`
		}
...
func (svc *service) ensureReady() error {
	...
	if _, err := os.Stat(filepath.Join(svc.dataDir, settingsFile)); errors.Is(err, os.ErrNotExist) {
		_, err = svc.saveSettings("Hello from v5-reference-app-go")
		return err
	}
	return nil
}
...
func (svc *service) saveSettings(message string) (appSettings, error) {
	...
	if settings.Message == "" {
		settings.Message = "Hello from v5-reference-app-go"
	}
...
func ensureWritable(dir string) error {
	path := filepath.Join(dir, ".fw-reference-write-test")
