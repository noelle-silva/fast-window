...
export function TracklessSlider(props: TracklessSliderProps) {
  ...
  return (
    <div className="trackless-slider">
      <div className="trackless-slider-header">
        <span className="trackless-slider-label">{label}</span>
        <output className="trackless-slider-value" aria-live="polite">{displayValue}</output>
      </div>

      <div
        ref={sliderRef}
        className="trackless-slider-field"
        role="slider"
        ...
        onKeyDown={onKeyDown}
      >
        <div className="trackless-slider-thumb" style={{ left: `${ratio * 100}%` }} aria-hidden="true" />
      </div>
    </div>
  )
}
