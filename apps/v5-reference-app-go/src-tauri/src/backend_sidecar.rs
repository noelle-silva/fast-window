...
impl BackendState {
    ...
    pub(crate) async fn endpoint(&self) -> Result<BackendEndpoint, String> {
        self.process.endpoint("v5 reference Go 后台未就绪").await
    }
}
...
pub(crate) async fn start_backend(
    ...
    let session_token = random_token("ref-backend");
...
fn spawn_backend_child(
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| format!("启动 v5 reference Go 后台失败: {e}"))?;
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| "v5 reference Go 后台 stdout 不可用".to_string())?;
    Ok((child, stdout))
}
...
fn resolve_backend_sidecar(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    let exe_name = if cfg!(target_os = "windows") {
        "v5-reference-app-go-backend.exe"
    } else {
        "v5-reference-app-go-backend"
    };
    let candidates = [
        std::env::current_exe()
            .ok()
    ...
    Err(format!("v5 reference Go 后台 sidecar 不存在: {exe_name}"))
}
