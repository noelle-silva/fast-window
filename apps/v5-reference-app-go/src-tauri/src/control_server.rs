...
pub(crate) const REFERENCE_APP_ID: &str = "v5-reference-app-go";
...
pub(crate) fn random_token(prefix: &str) -> String {
    ...
    getrandom::fill(&mut bytes).expect("failed to generate v5 reference app token");
        ...
        .spawn(move || {
            for stream in listener.incoming() {
                match stream {
                    ...
                    Err(error) => {
                        eprintln!(
                            "[v5-reference-app-go] {} connection failed: {error}",
                            server_name
                        );
                        break;
                    }
