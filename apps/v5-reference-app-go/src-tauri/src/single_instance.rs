...
pub(crate) fn start_single_instance_server(
    ...
    let token = random_token("ref-single");
    let endpoint = start_control_server(
        app,
        window_state,
        ControlServerConfig {
            name: "fw-v5-reference-app-go-single-instance",
...
fn write_instance_state(
    ...
    let state = InstanceState {
        app_id: REFERENCE_APP_ID.to_string(),
...
mod tests {
    ...
    use crate::control_server::REFERENCE_APP_ID;

    fn valid_state() -> InstanceState {
        InstanceState {
            app_id: REFERENCE_APP_ID.to_string(),
            desktop_identifier: "com.fastwindow.v5referenceappgo.dev".to_string(),
            server_id: SINGLE_INSTANCE_SERVER_ID.to_string(),
    ...
    fn validates_identifier_specific_state() {
        assert!(is_valid_instance_state(
            &valid_state(),
            "com.fastwindow.v5referenceappgo.dev"
        ));
        assert!(!is_valid_instance_state(
            &valid_state(),
            "com.fastwindow.v5referenceappgo"
        ));
    }
