...
const CONFIG_FILE: &str = "v5-reference-app-go-settings.json";
const WRITE_TEST_FILE: &str = ".fw-reference-write-test";
...
fn load_settings(app: &tauri::AppHandle) -> Result<ReferenceSettings, String> {
    let path = settings_path(app)?;
    if !path.is_file() {
        return Ok(ReferenceSettings::default());
    }
