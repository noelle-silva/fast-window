 ...
 * - 特别地：v3 不提供宿主 http.request 原语面，网络请求请由插件自行实现（例如 background 用 fetch）。
 *
...
export function assertV3TaskKindAllowed(ctx: PluginContext, kind: string) {
  ...
  const hint =
    kind === 'http.request'
      ? 'v3 不提供宿主 http.request 原语面：请在插件 background 中使用 fetch（或自建网络层），再通过 workspace 文件/自定义协议把结果回传给 UI'
  ...
  throw new PluginBridgeError(
    'BAD_REQUEST',
    `v3 不允许用 task.kind 承接宿主业务能力（kind=${JSON.stringify(kind)}）。${hint}`,
  )
}
