...
import { DIRECT_PLUGIN_RUNTIME_PROFILE } from './pluginProfiles'
...
export default function TrustedPluginView(props: Props) {
  return (
    <PluginFrameView
      {...props}
      runtime="ui"
      runtimeProfile={DIRECT_PLUGIN_RUNTIME_PROFILE}
      assetBaseUrl={buildPluginAssetBaseUrl(props.pluginId, props.pluginMain)}
      buildSdkCode={token => buildPluginSdkCode({ pluginId: props.pluginId, token, runtime: 'ui', apiVersion: props.apiVersion, sdkProfile: DIRECT_PLUGIN_RUNTIME_PROFILE.sdkProfile, exposeMeta: DIRECT_PLUGIN_RUNTIME_PROFILE.exposeMeta })}
    />
  )
}
