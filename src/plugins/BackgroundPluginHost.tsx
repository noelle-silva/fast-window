...
export default function BackgroundPluginHost(props: Props) {
  const { pluginId, pluginCode, apiVersion, requires } = props
  ...
  const onLoad = () => {
    ...
    bootTimerRef.current = window.setTimeout(() => {
      console.error(`[plugin-shell] bg boot timeout for "${pluginId}"`)
    }, 4000)
    ...
    const sdkCode = buildPluginSdkCode({ pluginId, token: tokenRef.current, runtime: 'background', apiVersion })
