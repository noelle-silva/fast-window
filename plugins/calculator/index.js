    ...
    async function ensureStorageReady() {
      ...
      storageReadyPromise = Promise.resolve()
        .then(async () => {
          ...
          // 若分片已存在（例如用户手动恢复），只补 meta
          const shardPath = keyToPath('state')
          ...
          const snapshot = {}
          if (obj && obj.state != null) snapshot.state = obj.state
          if (Object.keys(snapshot).length) {
            await writeJson(`_backup-migrate-${nowId()}.json`, snapshot).catch(() => {})
            await writeJson(shardPath, snapshot.state)
