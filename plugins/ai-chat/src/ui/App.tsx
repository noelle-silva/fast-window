...
function clampOffset(
  ...
  const sFit = isFinite(fit) && fit > 0 ? fit : 1
  const sZoom = isFinite(zoom) && zoom > 0 ? zoom : 1
  const s = sFit * sZoom

  if (!sw || !sh || !iw || !ih || !isFinite(s) || s <= 0) return offset

  const cw = iw * s
  const ch = ih * s

  const minX = cw >= sw ? sw - cw : 0
  const maxX = cw >= sw ? 0 : sw - cw
  const minY = ch >= sh ? sh - ch : 0
  const maxY = ch >= sh ? 0 : sh - ch

  return {
    x: clampNum(Number(offset?.x || 0), minX, maxX),
    y: clampNum(Number(offset?.y || 0), minY, maxY),
  }
}
