...
;(function () {
  ...
  function createToast() {
    ...
    function ensure() {
      ...
      el.id = '__fastWindowWebViewToast'
  ...
  function createCompatApi(baseApi) {
    ...
    async function ensureStorageReady() {
      ...
      storageReadyPromise = Promise.resolve()
        .then(async () => {
          ...
          const shardPath = keyToPath('items')
          ...
          const snapshot = {}
          if (obj && obj.items != null) snapshot.items = obj.items
          if (Object.keys(snapshot).length) {
            await writeJson(`_backup-migrate-${nowId()}.json`, snapshot).catch(() => {})
            await writeJson(shardPath, snapshot.items)
    ...
    function normalizeHttpReq(req) {
      ...
      const timeoutMs = r.timeoutMs == null ? null : Number(r.timeoutMs)
      const body = r.body == null ? null : String(r.body)
      const bodyBase64 = r.bodyBase64 == null ? null : String(r.bodyBase64)
      return { method, url, headers, timeoutMs, body, bodyBase64 }
    }
    ...
    const api = {
      ...
      ui: {
        ...
        openBrowserWindow: async (url) => {
          const u = String(url || '').trim()
          await tauri.invoke({ command: 'open_browser_window', payload: { url: u, pluginId: PLUGIN_ID } })
        },
      },
      ...
      net: {
        request: async (req) => {
          ...
          if (responseType === 'base64') {
            const { responseType: _rt, ...rest } = req || {}
            const r = normalizeHttpReq(rest)
            return tauri.invoke({ command: 'http_request_base64', payload: { req: r } })
          }
          const r = normalizeHttpReq(req)
          return tauri.invoke({ command: 'http_request', payload: { req: r } })
        },
      },
